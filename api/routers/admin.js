...
router.get('/stats', requireAuth, async (req, res) => {
    try {
        const [subscribers] = await pool.promise().query(
            'SELECT COUNT(*) as count FROM newsletter_subscribers'
        );
        const [campaigns] = await pool.promise().query(
            'SELECT COUNT(*) as count FROM email_campaigns WHERE sent_at IS NOT NULL'
        );
        const [lastCampaign] = await pool.promise().query(
...
router.put('/campaigns/:id', requireAuth, async (req, res) => {
    ...
    try {
        ...
        await pool.promise().query('DELETE FROM email_campaign_items WHERE campaign_id = ?', [req.params.id]);
...
router.delete('/campaigns/:id', requireAuth, async (req, res) => {
    try {
        ...
        await pool.promise().query('DELETE FROM email_campaign_items WHERE campaign_id = ?', [req.params.id]);
        await pool.promise().query('DELETE FROM email_campaigns WHERE id = ?', [req.params.id]);
    ...
    try {
        ...
        const [items] = await pool.promise().query(`
            SELECT
                ...
                p.price,
                p.description_en,
                CONCAT('http://localhost:3001/products/images/', p.image) as image_url
            FROM email_campaign_items eci
            JOIN products p ON eci.product_id = p.product_id
            WHERE eci.campaign_id = ?
            ORDER BY eci.sort_order ASC
